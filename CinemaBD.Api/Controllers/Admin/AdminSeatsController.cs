...
[ApiController, Authorize(Policy = "AdminOnly"), Route("api/admin/seats")]
public class AdminSeatsController : ControllerBase
{
    ...
    [HttpPost("{id}/toggle")]
    public async Task<IActionResult> Toggle(string id, [FromQuery] string? roomId, CancellationToken ct)
    {
        var status = await _service.ToggleStatusAsync(id, roomId, ct);
        return status == null
            ? NotFound(new ApiResponse<object>(false, "Không tìm thấy ghế.", null))
            : Ok(new ApiResponse<object>(true, "Cập nhật trạng thái ghế thành công", status));
    }
}
