...
public class AdminSeatsController : ControllerBase
{
    ...
    [HttpPost("{id}/toggle")] public async Task<IActionResult> Toggle(string id, CancellationToken ct) => Ok(new ApiResponse<object>(true, "OK", await _service.ToggleStatusAsync(id, ct)));
}
