...
public class AdminArticlesController : ControllerBase
{
    ...
    [HttpPost] public async Task<IActionResult> Create([FromBody] ArticleRequest request, CancellationToken ct) => Ok(new ApiResponse<object>(true, "OK", await _service.CreateAsync(request.Title, request.Summary, request.Content, request.ImageUrl, ct)));
    [HttpPut("{id:int}")] public async Task<IActionResult> Update(int id, [FromBody] ArticleRequest request, CancellationToken ct) => Ok(new ApiResponse<object>(true, "OK", await _service.UpdateAsync(id, request.Title, request.Summary, request.Content, request.ImageUrl, ct)));
